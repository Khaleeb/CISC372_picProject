...
// DEFINE NUMBER OF THREADS HERE, or comment it out and assign at compilation
#define N 4
...
//convolute_thread: Independent thread function of Convolute
//Parameters:
//Returns: 
    ...
    for (row = (tRank*threadRows); row < ((tRank+1)*threadRows); row++){
        for (pix=0;pix<srcImage->width;pix++){
            for (bit=0;bit<srcImage->bpp;bit++){
                destImage->data[Index(pix,row,srcImage->width,bit,srcImage->bpp)]=getPixelValue(srcImage,pix,row,bit,*args.algorithm);
            }
        }
    }
...
//            Ealgorithm: The kernel matrix to use for the convolution
//Returns: Nothing
void convolute(){
    args.nThreads = N;
